...
// script for star reset in the interview game

public class interStar : MonoBehaviour
{
    public GameObject clearStar, rightHand, leftHand;
    //public GameObject[] interviewies;
    ...
    {
        for (int i = 0; i < interviewies.Length; i++)
        {
            workerInitialPositions[i] = interviewies[i].transform.position;
            Debug.Log("Added worker " + interviewies[i] + " with position " + interviewies[i].transform.position);
        }
    }
    ...
    void ResetAll()
    {
        for (int i = 0; i < interviewies.Length; i++)
        {
            interviewies[i].transform.position = workerInitialPositions[i];
        }
    }
    */
    bool userLetGo()
